...
public class BaseEquipmentMBTests : TestCollection
{
	...
	{
		var equipmentMB = new GameObject("equipment").AddComponent<MockEquipmentMB>();
	...
	public IEnumerator OnValidateDuplicates()
	{
		...
		equipmentMB.equipment = new MockEquipment(
			new Record<EquipmentSlot, ItemMB>{ key = EquipmentSlot.MainHand, value = itemMBA },
			new Record<EquipmentSlot, ItemMB>{ key = EquipmentSlot.MainHand, value = itemMBB }
		);
	...
	public IEnumerator OnValidateSetSheetDoesNotThrowWhenValueNull()
	{
		...
		Assert.DoesNotThrow(() => equipmentMB.OnValidate());
	}
