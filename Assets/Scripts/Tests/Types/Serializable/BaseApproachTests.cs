...
public class BaseApproachTests : TestCollection
{
	...
	public void GetApproach() {
		...
		var routine = approach.Apply(obj.transform, Vector3.right, 0f);
		routine.MoveNext();
		routine.MoveNext();
	...
	public void GetApproachTimeDelta() {
		...
		var routine = approach.Apply(obj.transform, Vector3.right, 1);
		routine.MoveNext();
		routine.MoveNext();
	...
	public void GetApproachTerminates() {
		var obj = new GameObject("obj");
		var approach = new MockApproach();
		var routine = approach.Apply(obj.transform, Vector3.right, 0.51f);

		Assert.AreEqual(
			(true, true, false),
			(routine.MoveNext(), routine.MoveNext(), routine.MoveNext())
		);
	}
	...
	public void GetApproachOnPositionUpdatedCalled() {
		...
		var obj = new GameObject("obj");
		var approach = new MockApproach();
		var routine = approach.Apply(obj.transform, Vector3.right, 0.2f);
	...
	public void GetApproachOnPositionUpdatedAfterPositionUpdate() {
		...
		approach.onPositionUpdated = (in Transform t, in Vector3 _) =>
			called = t.position;
