...
public class UIOverlayFollowMB : MonoBehaviour
{
	...
	private float GetCamTargetDistance() {
		Vector3 camTargetDirection
			= this.Target.transform.position
			- this.ctrlCam!.transform.position;
		return Vector3.Project(
			camTargetDirection,
			this.ctrlCam.transform.forward
		).magnitude;
	}

	private float GetTargetMaxBoundSize() => this
		.Target
		.bounds
		.size
		.AsEnumerable()
	...
	private void LateUpdate() {
		...
		this.transform.position = this.ctrlCam!.WorldToScreenPoint(
			this.Target.transform.position
		);
